...
class Validators {
  ...
  getValidatorQuorums(sml, verified = false) {
    if (verified) {
      return this.removePoseQuorum(sml.getVerifiedQuorums().filter(quorum => quorum.llmqType === sml.getValidatorLLMQType()));
    }
    return this.removePoseQuorum(sml.getQuorums().filter(quorum => quorum.llmqType === sml.getValidatorLLMQType()));
  }
   ...
   * @return {QuorumEntry[]} - array of all active validator quorums
   */
  removePoseQuorum(validatorQuorums) {
